import { processDemoRequestSubmission } from '../src/server/demoRequestService.js';
...
export default async function handler(req: MinimalRequest, res: MinimalResponse) {
  try {
    ...
    const result = await processDemoRequestSubmission(parsedBody, {
      siteUrl: resolveRequestOrigin(req),
    });
    return res.status(result.status).json(result.body);
  } catch (error) {
    console.error('Unhandled demo request API error', error);
